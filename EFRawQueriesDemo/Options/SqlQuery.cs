...
public class SqlQuery
{
    ...
    public static async Task WithSimpleParameter()
    {
        ...
        var result = await dbContext.Database
            .SqlQuery<Movie>($"Select * from Movies where Id = {id}")
            .ToListAsync();

        result = await dbContext.Database
            .SqlQuery<Movie>($"GetMoviesById {id}")
            .ToListAsync();

        int? voteCount = null;

        result = await dbContext.Database
            .SqlQuery<Movie>($"Select * from Movies where VoteCount = {voteCount}")
            .ToListAsync();

        result = await dbContext.Database
            .SqlQuery<Movie>($"GetMoviesByVoteCount {voteCount}")
            .ToListAsync();
        ...
        result = await dbContext.Database
            .SqlQuery<Movie>($"GetMovies {p0}, {p1}, {p2}")
            .ToListAsync();
    }
