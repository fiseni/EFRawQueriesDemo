...
public class FromSql
{
    ...
    public static async Task WithSimpleParameter()
    {
        ...
        var result = await dbContext.Movies
            .FromSql($"Select * from Movies where Id = {id}")
            .ToListAsync();

        result = await dbContext.Movies
            .FromSql($"GetMoviesById {id}")
            .ToListAsync();

        int? voteCount = null;

        result = await dbContext.Movies
            .FromSql($"Select * from Movies where VoteCount = {voteCount}")
            .ToListAsync();

        result = await dbContext.Movies
            .FromSql($"GetMoviesByVoteCount {voteCount}")
            .ToListAsync();
