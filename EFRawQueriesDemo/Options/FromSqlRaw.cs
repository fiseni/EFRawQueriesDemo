...
public class FromSqlRaw
{
    ...
    public static async Task WithSimpleParameter()
    {
        ...
        var result = await dbContext.Movies
            .FromSqlRaw("Select * from Movies where Id = {0}", id)
            .ToListAsync();

        result = await dbContext.Movies
            .FromSqlRaw("GetMoviesById {0}", id)
            .ToListAsync();

        int? voteCount = null;

        result = await dbContext.Movies
            .FromSqlRaw("Select * from Movies where VoteCount = {0}", voteCount)
            .ToListAsync();

        result = await dbContext.Movies
            .FromSqlRaw("GetMoviesByVoteCount {0}", voteCount)
            .ToListAsync();
        ...
        result = await dbContext.Movies
            .FromSqlRaw("GetMovies {0}, {1}, {2}", p0, p1, p2)
            .ToListAsync();
    }
    ...
    public static async Task WithSqlParameter()
    {
        ...
        result = await dbContext.Movies
            .FromSqlRaw("GetMoviesById @id", id)
            .ToListAsync();
